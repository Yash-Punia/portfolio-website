...
const createHash = (matchPaths, components, cleanedClientVisitedPageComponents, notVisitedPageComponents, cleanedSSRVisitedPageComponents) => _crypto.default.createHash(`md5`).update(JSON.stringify({
  matchPaths,
  components,
...
const writeAll = async state => {
  ...
  let cleanedClientVisitedPageComponents = [];
  let notVisitedPageComponents = [];

  if (process.env.GATSBY_EXPERIMENTAL_LAZY_DEVJS) {
    var _state$visitedPages$g2;

    const clientVisitedPageComponents = [...(((_state$visitedPages$g2 = state.visitedPages.get(`client`)) === null || _state$visitedPages$g2 === void 0 ? void 0 : _state$visitedPages$g2.values()) || [])]; // Remove any page components that no longer exist.

    cleanedClientVisitedPageComponents = components.filter(component => clientVisitedPageComponents.some(pageComponentChunkName => pageComponentChunkName === component.componentChunkName)); // Get list of page components that the user has _not_ visited.

    notVisitedPageComponents = components.filter(component => // Filter out page components the user has visited.
    !cleanedClientVisitedPageComponents.some(c => c.componentChunkName === component.componentChunkName));
  }

  const newHash = createHash(matchPaths, components, cleanedClientVisitedPageComponents, notVisitedPageComponents, cleanedSSRVisitedPageComponents);
  ...
  \n\n`;
    lazySyncRequires += `exports.ssrComponents = {\n${cleanedSSRVisitedPageComponents.map(c => `  "${c.componentChunkName}": ${hotMethod}(preferDefault(require("${(0, _gatsbyCoreUtils.joinPath)(c.component)}")))`).join(`,\n`)}
  }\n\n`;
...
\n\n`;
  ...
  const preferDefault = m => (m && m.default) || m
  \n\n`;
    lazyClientSyncRequires += `exports.lazyComponents = {\n${cleanedClientVisitedPageComponents.map(c => `  "${c.componentChunkName}": ${hotMethod}(preferDefault(require("${(0, _gatsbyCoreUtils.joinPath)(c.component)}")))`).join(`,\n`)}
  }\n\n`; // Add list of not visited components.

    lazyClientSyncRequires += `exports.notVisitedPageComponents = {\n${notVisitedPageComponents.map(c => `  "${c.componentChunkName}": true`).join(`,\n`)}
  }\n\n`;
    (0, _gatsbyWebpackVirtualModules.writeModule)(`$virtual/lazy-client-sync-requires`, lazyClientSyncRequires);
  } else {
    (0, _gatsbyWebpackVirtualModules.writeModule)(`$virtual/lazy-client-sync-requires`, ``);
...
if (process.env.GATSBY_EXPERIMENTAL_LAZY_DEVJS) {
   ...
   * files as required
   */
  _redux.emitter.on(`CREATE_CLIENT_VISITED_PAGE`, () => {
